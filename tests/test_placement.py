...
class _StubRecipe:
    """Minimal stand-in for Recipe that the base method actually uses."""

    def __init__(self, defaults: dict, layout=None):
...
def _stub_runtime():
    """Build a minimal RuntimePlugin subclass for compute_required_nodes tests."""
...
def test_compute_required_nodes_legacy_path_one_gpu_per_host():
    """No cluster arg -> total_gpus (legacy 1 GPU/host)."""
    runtime = _stub_runtime()
    recipe = _StubRecipe({"tensor_parallel": 4})
    assert runtime.compute_required_nodes(recipe) == 4


def test_compute_required_nodes_cluster_aware_multi_gpu_host():
    """With cluster, an 8-GPU host satisfies tp=8 with 1 host."""
    from sparkrun.core.cluster_manager import ClusterDefinition

    runtime = _stub_runtime()
    recipe = _StubRecipe({"tensor_parallel": 8})
    cluster = ClusterDefinition(
        name="h200-box",
        hosts=["dgx-h200"],
        hosts_hardware={"dgx-h200": _h200_8gpu()},
    )
    assert runtime.compute_required_nodes(recipe, cluster=cluster) == 1


def test_compute_required_nodes_returns_none_when_unconfigured():
    runtime = _stub_runtime()
    recipe = _StubRecipe({})
    assert runtime.compute_required_nodes(recipe) is None


def test_compute_required_nodes_cluster_aware_dgx_unchanged():
    """Cluster-aware call on a homogeneous DGX cluster matches legacy answer."""
    from sparkrun.core.cluster_manager import ClusterDefinition

    runtime = _stub_runtime()
    recipe = _StubRecipe({"tensor_parallel": 3})
    cluster = ClusterDefinition(name="dgx3", hosts=["s1", "s2", "s3"])
    assert runtime.compute_required_nodes(recipe) == 3
    assert runtime.compute_required_nodes(recipe, cluster=cluster) == 3
