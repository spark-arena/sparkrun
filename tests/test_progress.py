...
def test_progress_heartbeat_reports_while_work_runs(caplog):
    """A silent long operation still says it is alive."""
    import logging
    import time

    from sparkrun.core.progress import PROGRESS, progress_heartbeat

    logger = logging.getLogger("sparkrun.test.heartbeat")
    with caplog.at_level(PROGRESS, logger="sparkrun.test.heartbeat"):
        with progress_heartbeat(logger, "Building image", interval=0.05):
            time.sleep(0.18)

    messages = [r.getMessage() for r in caplog.records]
    assert messages, "heartbeat emitted nothing while work was running"
    assert all("Building image" in m and "still running" in m for m in messages)


def test_progress_heartbeat_stops_at_exit(caplog):
    """The thread is joined on exit, so nothing is emitted afterwards."""
    import logging
    import time

    from sparkrun.core.progress import PROGRESS, progress_heartbeat

    logger = logging.getLogger("sparkrun.test.heartbeat.stop")
    with caplog.at_level(PROGRESS, logger="sparkrun.test.heartbeat.stop"):
        with progress_heartbeat(logger, "Capturing", interval=0.05):
            time.sleep(0.12)
        emitted = len(caplog.records)
        time.sleep(0.15)
...
def test_progress_heartbeat_propagates_the_failure_it_was_watching():
    ...
    logger = logging.getLogger("sparkrun.test.heartbeat.raise")
    with _pytest.raises(RuntimeError, match="build failed"):
        with progress_heartbeat(logger, "Building", interval=10.0):
