...
class TestFetchUpstreamWheelHashes:
    ...
    def test_parses_both_hashes(self):
        vllm_resp = self._mock_release("Prebuilt vLLM Wheels (0.18.1rc1.dev121+gcd7643015.d20260325.cu132)")
        fi_resp = self._mock_release("Prebuilt FlashInfer Wheels (0.6.7-ede7a275-d20260325)")
...
class TestGetImageIdOnHost:
    def test_returns_id_on_success(self):
        with mock.patch(
            "sparkrun.orchestration.primitives.run_script_on_host", return_value=_remote_result(True, stdout="sha256:abc123\n")
        ):
            assert EugrBuilder._get_image_id_on_host("img", "host1") == "sha256:abc123"

    def test_returns_none_on_failure(self):
        with mock.patch("sparkrun.orchestration.primitives.run_script_on_host", return_value=_remote_result(False)):
            assert EugrBuilder._get_image_id_on_host("img", "host1") is None

    def test_returns_none_on_empty_stdout(self):
        with mock.patch("sparkrun.orchestration.primitives.run_script_on_host", return_value=_remote_result(True, stdout="  \n")):
            assert EugrBuilder._get_image_id_on_host("img", "host1") is None


class TestGetRepoHeadOnHost:
    def test_returns_commit_on_success(self):
        with mock.patch("sparkrun.orchestration.primitives.run_script_on_host", return_value=_remote_result(True, stdout="abc123def456\n")):
...
class TestCanSkipBuild:
    def test_cache_hit_skips_build(self, tmp_path):
        ...
        with (
            mock.patch("sparkrun.containers.registry.get_image_id", return_value="sha256:deadbeef"),
...
class TestCanSkipBuildDelegated:
    def test_delegated_cache_hit(self, tmp_path):
        ...
        with (
            mock.patch.object(EugrBuilder, "_get_image_id_on_host", return_value="sha256:deadbeef"),
            ...
            assert builder._can_skip_build("sparkrun-eugr-vllm", [], config, host="head1", ssh_kwargs={}) is True
    ...
    def test_delegated_cache_miss_no_entry(self, tmp_path):
        ...
        assert builder._can_skip_build("sparkrun-eugr-vllm", [], config, host="head1", ssh_kwargs={}) is False
...
class TestSaveBuildMetadata:
    def test_saves_full_metadata(self, tmp_path):
        ...
        with (
            mock.patch("subprocess.run", side_effect=[git_result, docker_result]),
...
class TestSaveBuildMetadataDelegated:
    def test_saves_with_host_key(self, tmp_path):
        ...
        with (
            mock.patch.object(EugrBuilder, "_get_repo_head_on_host", return_value="abc123def456"),
            mock.patch(
                "sparkrun.orchestration.primitives.run_script_on_host", return_value=_remote_result(True, stdout="vllm_commit: cd7643015\n")
            ),
            mock.patch.object(
                builder,
                "process_version_info",
    ...
    def test_handles_ssh_failure_gracefully(self, tmp_path):
        ...
        with (
            mock.patch.object(EugrBuilder, "_get_repo_head_on_host", return_value=None),
            mock.patch("sparkrun.orchestration.primitives.run_script_on_host", return_value=_remote_result(False)),
