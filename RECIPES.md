...
#### Explicit KV allocation

For vLLM, put a fixed per-GPU KV budget in runtime defaults:
...
`8G` means 8 GiB (binary); `8g` means 8 billion bytes (decimal). Raw positive
integer bytes are also accepted. CLI overrides take precedence. A custom command
can use `--kv-cache-memory-bytes {kv_cache_memory_bytes}`; the estimator also
recognizes a literal budget in that flag or the `--kv-cache-memory` alias.
Invalid explicit budgets fail planning rather than silently using an estimate.

This budget is allocated **on every GPU**: it is not divided by TP or PP, nor
multiplied by request count. The reported per-GPU total is sharded weights plus
that allocation. Estimated context demand is shown separately, and
`gpu_memory_utilization` does not determine the KV allocation when explicit bytes
are supplied. Runtime validation still checks whether the context fits.

For hybrid linear/MLA models, the generic MLA estimator cannot size the whole
cache. Such models report unknown context capacity instead of treating every
layer as MLA. An explicit KV budget still gives a useful allocation estimate.
See [GPU allocation admission](docs/MULTIPLATFORM.md#gpu-allocation-admission)
for the distinction between memory fit and exclusive/shared placement.
...
| `layer_types`       | Per-layer attention types; mixed linear/MLA cache sizing is reported as unknown.          |
